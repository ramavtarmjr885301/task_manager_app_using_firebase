...
///
///
// Filename: edit_task_screen.dart (UPDATED for Numeric Priority Ranking)
  ...
  Future<void> _loadTaskData() async {
    try {
      ...
      if (docSnapshot.exists) {
        ...
        _descriptionController.text = data['description'] ?? '';
        _currentStatus = data['status'] ?? 'pending';
        // priority is loaded for the dropdown
        _selectedPriority = data['priority'] ?? 'medium'; 
        _selectedCategory = data['category'] ?? 'Personal';
      ...
      try {
        ...
        final updatedTaskData = {
          ...
          'dueDate': dueDateTime != null
              ...
              : null,
          'priority': _selectedPriority,
          'priorityRank': _getPriorityRank(_selectedPriority), // <<< NEW FIELD UPDATED
          'category': _selectedCategory,
          'status': _currentStatus,
          'updatedAt': FieldValue.serverTimestamp(),
        };
